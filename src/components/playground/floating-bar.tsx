...
export function FloatingBar({
  ...
  // Track last synced instruction to avoid resetting the editor on every keystroke
  ...
  const selectMention = useCallback(
    (img: MentionImage, state: InternalMentionState) => {
      ...
      // Build: text before @, the mention span, text after
      ...
      const afterNode = document.createTextNode("\u00A0" + after); // nbsp keeps cursor visible
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
    ...
    // Prevent bare Enter from creating <div> line breaks
    if (e.key === "Enter" && !e.metaKey && !e.ctrlKey) {
      e.preventDefault();
    }
  }
