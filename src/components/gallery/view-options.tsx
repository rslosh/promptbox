...
export function ViewOptions({
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full z-50 mt-2 w-52 rounded-xl border border-hairline gos-glass p-4 shadow-xl backdrop-blur-xl">
          <div className="space-y-4">
            {/* Layout */}
            <div>
              <h4 className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-secondary">
                Layout
              </h4>
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        layout === value ? "border-accent bg-accent" : "border-strong"
                      )}
                    >
                      {layout === value && (
                ...
                {(["small", "medium", "large"] as const).map((size) => (
                  ...
                  >
                    <div
                      className={cn(
                        "flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2",
                        imageSize === size ? "border-accent bg-accent" : "border-strong"
                      )}
                    >
                      {imageSize === size && (
                        <div className="h-1.5 w-1.5 rounded-full bg-surface" />
